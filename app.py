...
# Config e imports
# -------------------------
sys.path.append("src")  # mantém import do seu dataset/config
...
st.markdown("Upload de **print do gráfico (1m)** e leitura probabilística.")
...
if uploaded:
    ...
    x = preprocess(img)
    if x is None:
        st.error("Erro no crop. Ajuste os valores em config.py.")
    else:
        with torch.no_grad():
